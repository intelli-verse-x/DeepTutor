...
# ─────────────────────────────────────────────────────────────────────────
# Fork compatibility shim — IntelliVerseX / QuizVerse "/memory" contract
# ─────────────────────────────────────────────────────────────────────────
# The platform talks to a simple per-user memory API (summary + profile).
# Pre-merge this was backed by a fork-only PostgreSQL ``MemoryService`` that no
# longer exists. We re-home the same HTTP contract onto upstream's three-layer
# ``MemoryStore``:
#       summary  ->  L3 "recent"
#       profile  ->  L3 "profile"
# Per-user isolation comes from the ``x-user-id`` -> ``multi_user`` bridge in
# ``TenantMiddleware``, so each tenant reads/writes its own scoped files.
# These routes (``""``, ``/refresh``, ``/clear``) do not collide with the v3
# workbench routes above.

from deeptutor.api.middleware.tenant import require_user_id  # noqa: E402

CompatMemoryFile = Literal["summary", "profile"]
_COMPAT_SLOT_MAP: dict[str, str] = {"summary": "recent", "profile": "profile"}
_COMPAT_VALID_FILES = set(_COMPAT_SLOT_MAP)


def _compat_read(slot: str) -> str:
    try:
        return get_memory_store().read_raw("L3", slot) or ""
    except Exception:  # noqa: BLE001
        return ""


def _compat_updated_at(slot: str) -> str | None:
    try:
...
def _compat_snapshot() -> dict:
    return {
        "summary": _compat_read("recent"),
        "profile": _compat_read("profile"),
        "summary_updated_at": _compat_updated_at("recent"),
        "profile_updated_at": _compat_updated_at("profile"),
    }


class CompatFileUpdateRequest(BaseModel):
    file: CompatMemoryFile
    content: str = ""


class CompatMemoryRefreshRequest(BaseModel):
    session_id: str | None = None
    language: str = "en"


class CompatMemoryClearRequest(BaseModel):
    file: CompatMemoryFile | None = None


@router.get("")
async def get_memory_compat(user_id: str = Depends(require_user_id)):
    """Per-user memory snapshot (summary + profile). Fork/QuizVerse contract."""
    return _compat_snapshot()


@router.put("")
async def update_memory_compat(
    payload: CompatFileUpdateRequest,
    user_id: str = Depends(require_user_id),
):
    if payload.file not in _COMPAT_VALID_FILES:
        raise HTTPException(status_code=400, detail=f"Invalid file: {payload.file}")
    await get_memory_store().overwrite_doc(
        "L3", _COMPAT_SLOT_MAP[payload.file], payload.content
    )
    return {**_compat_snapshot(), "saved": True}


@router.post("/refresh")
async def refresh_memory_compat(
    payload: CompatMemoryRefreshRequest,
    user_id: str = Depends(require_user_id),
):
    """Re-consolidate the per-user "recent" summary from the L1 activity trace.

    NOTE: upstream consolidates memory from the L1 trace, not from a single
    ``session_id`` as the legacy fork API did. ``session_id`` is accepted for
    backward compatibility but is advisory only.
    """
    changed = False
    try:
        from deeptutor.services.memory.consolidator import run_update

        await run_update("L3", "recent", language=payload.language or "en")
        changed = True
    except Exception as exc:  # noqa: BLE001
        logger.warning("memory compat refresh failed: %s", exc)
    return {**_compat_snapshot(), "changed": changed}


@router.post("/clear")
async def clear_memory_compat(
    payload: CompatMemoryClearRequest | None = None,
    user_id: str = Depends(require_user_id),
):
    target = payload.file if payload else None
    if target and target not in _COMPAT_VALID_FILES:
        raise HTTPException(status_code=400, detail=f"Invalid file: {target}")
    store = get_memory_store()
    slots = [_COMPAT_SLOT_MAP[target]] if target else list(_COMPAT_SLOT_MAP.values())
    for slot in slots:
        await store.overwrite_doc("L3", slot, "")
    return {**_compat_snapshot(), "cleared": True}
